...
=== "S1_Burst"
    ...
    扩展自 `ASF_Base_Downloader`，用于 ASF 的 **SLC-BURST** 数据集。一个 burst 约为完整 IW 切片的 1/9，因此面向 AOI 的 burst 堆叠比等效的 `S1_SLC` 搜索拉取的数据少得多。请与 `ISCE3_Burst` 处理器和 `ISCE3_Dolphin_S1_PL` 分析器配合使用。

    与 `S1_SLC` 相比只有 `download()` 不同：它将选中的颗粒交给 `burst2safe`，由后者组装成合法的 `.SAFE` 目录。
...
=== "NISAR_GSLC"
    ...
    通过 ASF 搜索并下载 NISAR **L2 GSLC**（已地理编码的 SLC）产品——每个日期一帧的已地理编码复数 SLC，可直接送入 `ISCE3_NISAR` 处理器与 `ISCE3_Dolphin_NISAR_PL` 分析器。搜索、筛选、足迹与配对选择均复用 `ASF_Base_Downloader`；**不下载轨道**：NISAR 产品自带状态矢量。
