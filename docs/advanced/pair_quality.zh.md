...
**相干性惩罚** — 偏移二次函数，γ ≥ 0.40 时零惩罚，γ ≤ 0.10 时满惩罚：
...
该阈值针对**真实（未滤波）相干性**校准，数据源为全球 S1 季节性相干性数据集
（Kellndorfer et al. 2022）：其 12 天中位数约为 0.31，6 天中位数约为 0.42，
因此 0.40 为可用带的高端。（0.60 适用于经 Goldstein 滤波后的相干性，滤波会抬高数值。）

**好 / 一般 / 差** 显示阈值同样基于真实相干性：好 ≥ 0.40，一般 0.25–0.40，差 < 0.25。
